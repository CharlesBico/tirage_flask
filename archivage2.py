...
class ArchivageApp(ctk.CTk):
    def __init__(self):
        super().__init__()
        self.title("Application d'archivage électronique/MINHAS/Direction Régionale du Worodougou")
    ...
    def create_widgets(self):
        ...
        ctk.CTkLabel(frame_filter, text="Filtrer par nom").pack(side="left", padx=5)
        self.filter_nom = ctk.CTkEntry(frame_filter, width=200)
        self.filter_nom.pack(side="left", padx=5)

        ctk.CTkLabel(frame_filter, text="Filtrer par catégorie").pack(side="left", padx=5)
        self.filter_categorie = ctk.CTkEntry(frame_filter, width=200)
        self.filter_categorie.pack(side="left", padx=5)

        ctk.CTkButton(frame_filter, text="Appliquer", command=self.filtrer).pack(side="left", padx=10)
        ctk.CTkButton(frame_filter, text="Réinitialiser", command=self.load_data).pack(side="left")
    ...
    def enregistrer(self):
        ...
        cur.execute(
            ...
            (self.ent_nom.get(), self.ent_chemin.get(), self.ent_categorie.get(), self.ent_description.get())
        )
        conn.commit()
        conn.close()
    ...
    def modifier(self):
        ...
        cur = conn.cursor()
        cur.execute(
            "UPDATE documents SET nom=?, chemin=?, categorie=?, description=? WHERE id=?",
